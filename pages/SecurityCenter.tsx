    ...
    <div className="space-y-6 animate-fade-in pb-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col items-center justify-center text-center">
             <div className="relative w-32 h-32 flex items-center justify-center mb-4">
                ...
                <div className="text-3xl font-bold text-slate-800 dark:text-white">92</div>
             </div>
             <h3 className="font-bold text-slate-700 dark:text-white">Security Health Score</h3>
             <p className="text-sm text-slate-500 dark:text-slate-400">2 minor vulnerabilities detected</p>
          </div>

          {/* MFA Adoption */}
          <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
             <h3 className="font-bold text-slate-800 dark:text-white mb-4">MFA Adoption Rate</h3>
             <div className="flex items-center gap-4">
                ...
                <div>
                   <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-sm text-slate-600 dark:text-slate-300">Enabled (85%)</span>
                   </div>
                   <div className="flex items-center gap-2">
                      <div className="w-3 h-3 bg-red-500 rounded-full" />
                      <span className="text-sm text-slate-600 dark:text-slate-300">Disabled (15%)</span>
                   </div>
                   <button onClick={() => setShowMfaModal(true)} className="mt-3 text-xs text-indigo-600 dark:text-indigo-400 hover:underline font-bold">Configure My MFA</button>
                </div>
             </div>
          </div>

          {/* Recent Alerts */}
          <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
             ...
             <div className="space-y-3">
                {VULNERABILITIES.map((vuln, i) => (
                   <div key={i} className="p-3 bg-slate-50 dark:bg-slate-750 rounded-lg text-sm">
                      <div className="flex justify-between font-bold text-slate-700 dark:text-slate-200">
                         {vuln.issue}
                         <span className={`text-[10px] px-2 py-0.5 rounded ${vuln.severity === 'MEDIUM' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400'}`}>{vuln.severity}</span>
                      </div>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{vuln.remediation}</p>
                   </div>
                ))}
             </div>
          </div>
      ...
      {activeTab === 'rbac' && (
         <div className="space-y-6">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
               <div className="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-750 flex justify-between items-center">
                  <h3 className="font-bold text-slate-800 dark:text-white">Role Definitions</h3>
                  <button className="text-sm bg-slate-800 dark:bg-slate-600 text-white px-3 py-1.5 rounded-lg hover:bg-slate-700 dark:hover:bg-slate-500">
                     Add Custom Role
                  </button>
               </div>
               <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                     {INITIAL_ROLES.map((role) => (
                        <tr key={role.role} className="hover:bg-slate-50 dark:hover:bg-slate-750 transition-colors">
                           <td className="px-6 py-4 font-bold text-slate-700 dark:text-white">{role.role}</td>
                           <td className="px-6 py-4 flex items-center text-slate-700 dark:text-slate-300">
                              <Users className="w-4 h-4 mr-2 text-slate-400" /> {role.usersCount}
                           </td>
                           <td className="px-6 py-4">
                              <div className="flex gap-1 flex-wrap">
                                 {role.permissions.map(p => (
                                    <span key={p} className="text-[10px] bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300">{p}</span>
                                 ))}
                              </div>
                           </td>
                           <td className="px-6 py-4 text-right">
                              <button className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-medium text-xs">Edit Permissions</button>
                           </td>
                        </tr>
                     ))}
                  </tbody>
               ...
               <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex gap-4">
                  <div className="relative flex-1">
                     ...
                     <input 
                        ...
                        className="w-full pl-9 pr-4 py-2 border border-slate-200 dark:border-slate-600 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white dark:bg-slate-700 dark:text-white"
                        value={logFilter}
                        onChange={(e) => setLogFilter(e.target.value)}
                     />
                  </div>
                  <button className="flex items-center px-4 py-2 border border-slate-200 dark:border-slate-600 rounded-lg text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                     {filteredLogs.map((log) => (
                        <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-750 transition-colors">
                           <td className="px-6 py-3 text-slate-500 dark:text-slate-400 font-mono text-xs">{log.timestamp}</td>
                           <td className="px-6 py-3">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                                 log.severity === 'CRITICAL' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                                 log.severity === 'WARNING' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' :
                                 'bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300'
                              }`}>
                                 {log.severity}
                              </span>
                           </td>
                           <td className="px-6 py-3 font-medium text-slate-700 dark:text-slate-200">{log.userName}</td>
                           <td className="px-6 py-3 text-slate-600 dark:text-slate-300">{log.action}</td>
                           <td className="px-6 py-3 text-slate-500 dark:text-slate-400">{log.resource}</td>
      ...
      {activeTab === 'compliance' && (
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
               ...
               <div className="space-y-4">
                  {MOCK_COMPLIANCE_REQUESTS.map((req) => (
                     <div key={req.id} className="border border-slate-200 dark:border-slate-700 rounded-lg p-4 flex justify-between items-center">
                        <div>
                           <div className="flex items-center gap-2 mb-1">
                              <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                                 req.type === 'DELETE_DATA' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'
                              }`}>
                                 {req.type.replace('_', ' ')}
                              </span>
                              <span className="text-xs text-slate-400">{req.requestDate}</span>
                           </div>
                           <p className="font-bold text-sm text-slate-800 dark:text-white">{req.clientName} (ID: {req.clientId})</p>
                        </div>
                        {req.status === 'PENDING' ? (
                           <button className="px-3 py-1 bg-slate-800 dark:bg-slate-700 text-white text-xs rounded hover:bg-slate-700 dark:hover:bg-slate-600">
                              Process Request
                           </button>
                        ) : (
            ...
            <div className="space-y-6">
               <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
                  <h3 className="font-bold text-slate-800 dark:text-white mb-2">Automated Data Retention</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Policies actively managing data lifecycle.</p>
               ...
               <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-sm border border-emerald-100 dark:border-emerald-900 text-left animate-fade-in">
                  ...
                  <div className="space-y-4">
                     <div>
                        ...
                        <div className="font-mono text-slate-700 dark:text-slate-200">James Robinson</div>
                     </div>
                     <div>
      ...
      {showMfaModal && (
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in transition-colors">
               <div className="p-6 text-center">
                  <div className="w-12 h-12 bg-indigo-100 dark:bg-indigo-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
                     <Smartphone className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />
                  </div>
                  <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">Setup 2-Factor Authentication</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Secure your account by linking an authenticator app.</p>
                  ...
                  {mfaStep === 1 ? (
                     ...
                     <div className="space-y-4">
                        ...
                        <input 
                           ...
                           className="w-full text-center tracking-[0.5em] font-bold text-xl py-3 border border-slate-200 dark:border-slate-600 rounded-lg focus:outline-none focus:border-indigo-500 bg-white dark:bg-slate-700 dark:text-white"
                           maxLength={6}
                           value={mfaCode}
               ...
               <div className="bg-slate-50 dark:bg-slate-750 p-4 text-center">
                  <button onClick={() => { setShowMfaModal(false); setMfaStep(1); }} className="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200">Cancel</button>
               </div>
            </div>
         </div>
      )}
