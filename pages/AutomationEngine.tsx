    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-4">
          <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-slate-100 flex flex-col items-center">
             ...
             <span className="text-xl font-bold text-teal-600">142h</span>
          </div>
          <div className="bg-white px-4 py-2 rounded-lg shadow-sm border border-slate-100 flex flex-col items-center">
             <span className="text-xs text-slate-400 uppercase font-bold">Active Rules</span>
             <span className="text-xl font-bold text-indigo-600">{workflows.filter(w => w.isActive).length}</span>
          </div>
        </div>
      </div>
      ...
      {activeTab === 'workflows' && (
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
            ...
            <div className="flex gap-3">
              <input 
                ...
                className="flex-1 p-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-teal-500 focus:outline-none"
                value={goalInput}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {workflows.map((wf) => (
              <div key={wf.id} className={`bg-white rounded-xl shadow-sm border p-6 transition-all ${wf.isActive ? 'border-slate-200' : 'border-slate-100 opacity-75'}`}>
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h4 className="text-lg font-bold text-slate-800">{wf.name}</h4>
                    <p className="text-sm text-slate-500">{wf.description}</p>
                  </div>
                  <button onClick={() => toggleWorkflow(wf.id)} className={`transition-colors ${wf.isActive ? 'text-teal-600' : 'text-slate-300'}`}>
                    {wf.isActive ? <PlayCircle className="w-8 h-8" /> : <PauseCircle className="w-8 h-8" />}
                  </button>
                </div>

                {/* Visual Flow */}
                <div className="bg-slate-50 rounded-lg p-4 space-y-3 relative overflow-hidden">
                  <div className="flex items-center text-sm font-medium text-slate-700">
                    <Zap className="w-4 h-4 mr-2 text-amber-500" />
                    WHEN: {wf.trigger}
                  </div>
                  {wf.conditions.map((cond, i) => (
                    <div key={i} className="flex items-center text-xs text-slate-500 ml-6">
                      <ArrowRight className="w-3 h-3 mr-2" />
                      IF {cond.field} is {cond.operator} {cond.value}
                    </div>
                  ))}
                  {wf.actions.map((act, i) => (
                    <div key={i} className="flex items-center text-sm font-medium text-slate-700 ml-6 pl-2 border-l-2 border-teal-200">
      ...
      {activeTab === 'email-ai' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[calc(100vh-250px)]">
          ...
          <div className="flex flex-col h-full bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            ...
            <textarea 
              className="flex-1 p-4 border border-slate-200 rounded-lg resize-none focus:ring-2 focus:ring-blue-500 focus:outline-none bg-slate-50"
          ...
          {/* Output */}
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 overflow-y-auto">
            {!emailAnalysis ? (
              ...
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="flex flex-wrap gap-3">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                    emailAnalysis.priority === 'HIGH' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'
                  }`}>
                ...
                <div className="space-y-2">
                  <h4 className="text-sm font-bold text-slate-700 uppercase">AI Suggested Draft</h4>
                  <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 whitespace-pre-wrap leading-relaxed">
                    {emailAnalysis.suggestedResponse}
                  </div>
                  <div className="flex gap-2">
                     <button className="text-xs text-blue-600 hover:underline">Copy to Clipboard</button>
                     <button className="text-xs text-blue-600 hover:underline">Edit Draft</button>
                  </div>
                </div>

                {/* Actions */}
                <div className="space-y-2">
                  <h4 className="text-sm font-bold text-slate-700 uppercase">Recommended Actions</h4>
      ...
      {activeTab === 'scheduler' && (
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-8 text-center">
          ...
          <h3 className="text-xl font-bold text-slate-800">Smart Batch Scheduler</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 max-w-4xl mx-auto text-left">
            {[0, 1, 2, 3, 4].map((day) => (
              <div key={day} className={`border rounded-lg p-4 ${day === 2 ? 'border-teal-500 bg-teal-50' : 'border-slate-200'}`}>
                <div className="text-xs font-bold text-slate-500 uppercase mb-2">
                  {['Mon', 'Tue', 'Wed', 'Thu', 'Fri'][day]}
                </div>
                {day === 2 ? (
                  <div className="space-y-2">
                     <div className="bg-white p-2 rounded border border-teal-200 text-xs shadow-sm">
                       <span className="font-bold text-teal-700">Batch #402</span>
                       <div className="text-slate-500">24 Letters</div>
                     </div>
                     <div className="bg-white p-2 rounded border border-teal-200 text-xs shadow-sm">
                       <span className="font-bold text-teal-700">Batch #403</span>
                       <div className="text-slate-500">12 Letters</div>
                     </div>
                  </div>
                ) : day === 4 ? (
                  <div className="text-xs text-slate-400 italic">No batches scheduled (Holiday)</div>
                ) : (
                  <div className="bg-slate-50 p-2 rounded text-xs text-slate-400">
                    Optimizing...
                  </div>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
             ...
             <div className="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center hover:bg-slate-50 transition-colors relative cursor-pointer">
               ...
               <p className="text-sm text-slate-600 font-medium">Upload Document</p>
               <p className="text-xs text-slate-400 mt-1">Simulate incoming client files or bureau letters</p>
             </div>

             {isClassifying && (
               <div className="mt-6 flex items-center justify-center text-indigo-600 text-sm">
            ...
            {!docClassification ? (
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 animate-fade-in">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="font-bold text-slate-800">Classification Results</h3>
                ...
                <div className="bg-slate-50 p-4 rounded-lg mb-6 border border-slate-200">
                  ...
                  <p className="text-lg font-bold text-slate-800">{docClassification.category}</p>
                </div>
                         ...
                         <div className="divide-y divide-slate-100 bg-white">
                            {bureauResponse.outcomes.map((item, i) => (
                              <div key={i} className="p-3 flex justify-between items-center">
                                <div>
                                  <p className="text-sm font-medium text-slate-800">{item.creditor}</p>
                                  <p className="text-xs text-slate-500">#{item.accountNumber}</p>
                                </div>
                                <span className={`px-2 py-1 rounded text-xs font-bold ${
                                  item.outcome === 'DELETED' ? 'bg-green-100 text-green-700' :
                                  item.outcome === 'VERIFIED' ? 'bg-red-100 text-red-700' :
      ...
      {activeTab === 'logs' && (
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="p-6 border-b border-slate-100 flex justify-between items-center">
             <h3 className="font-bold text-slate-800">Automation Execution Logs</h3>
